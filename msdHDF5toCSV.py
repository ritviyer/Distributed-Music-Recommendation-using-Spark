...
def main():
    outputFile1 = open('SongCSV.csv', 'w', encoding="utf-8")
    csvRowString = ""
    ...
    if prompt == True:
        while prompt:
            ...
            csvAttributeString = raw_input("\n\nIn what order would you like the colums of the CSV file?\n" +
                ...
                " ArtistName, Danceability, Duration, KeySignature, KeySignatureConfidence, Tempo, TrackID" +
    ...
    #the user, 
        ...
        csvRowString = ("SongID,AlbumName,ArtistName,Danceability,Duration,Tempo,Title,Year,TrackID")
        #################################################
    ...
    for root, dirs, files in os.walk(basedir):        
        ...
        for f in files:
            ...
            song = Song(str(hdf5_getters.get_song_id(songH5File).decode("utf-8")))
            ...
            song.artistID = str(hdf5_getters.get_artist_id(songH5File).decode("utf-8"))
            song.albumID = str(hdf5_getters.get_release_7digitalid(songH5File))
            song.albumName = str(hdf5_getters.get_release(songH5File).decode("utf-8"))
            song.artistLatitude = str(hdf5_getters.get_artist_latitude(songH5File))
            song.artistLocation = str(hdf5_getters.get_artist_location(songH5File).decode("utf-8"))
            song.artistLongitude = str(hdf5_getters.get_artist_longitude(songH5File))
            song.artistName = str(hdf5_getters.get_artist_name(songH5File).decode("utf-8"))
            ...
            song.title = str(hdf5_getters.get_title(songH5File).decode("utf-8"))
            song.year = str(hdf5_getters.get_year(songH5File))
            song.track_id = str(hdf5_getters.get_track_id(songH5File).decode("utf-8"))
